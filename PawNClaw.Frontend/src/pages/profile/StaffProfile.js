...
export default function StaffProfile() {
  ...
  useEffect(() => {
    async function fetchProfileData() {
      const { data } = await axios.get(`/api/staffs/${accountInfo.id}`);
      return data;
    }
    ...
    fetchProfileData().then((data) => {
      const profile = {
        ...
        avatarUrl: data.photos?.length > 0 ? data.photos[0].url : '',
      };
  ...
  const onSubmit = async () => {
    try {
      await updateStaffProfile(accountInfo.id, values).then(() => {
            ...
            <Grid item xs={12} md={8}>
              <Card sx={{ p: 3 }}>
                ...
                <Stack direction="row" alignItems="flex-end" justifyContent="flex-end" spacing={3} sx={{ mt: 3 }}>
                  <Button to={PATH_DASHBOARD.bookingList.list} color="error" variant="contained" component={RouterLink}>
                    Hủy
                  </Button>
...
// ----------------------------------------------------------------------
async function updateStaffProfile(profileId, values) {
  await axios.put(`/api/staffs/${profileId}`, {
    id: profileId,
    name: values.name,
    phone: values.phone,
  });
