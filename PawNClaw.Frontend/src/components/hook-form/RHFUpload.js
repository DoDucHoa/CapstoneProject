...
// type
import { UploadAvatar, UploadMultiFile, UploadSingleFile, UploadPhoto } from '../upload';
...
export function RHFUploadPhoto({ name, ...other }) {
  const { control } = useFormContext();
  ...
  return (
    <Controller
      ...
      render={({ field, fieldState: { error } }) => {
        ...
        return (
          <div>
            <UploadPhoto error={checkError} {...other} file={field.value} />
            {checkError && (
              <FormHelperText error sx={{ px: 2, textAlign: 'center' }}>
                {error.message}
              </FormHelperText>
